...
/* JADX INFO: loaded from: WormholeXTreme.jar:de/luricos/bukkit/WormholeXTreme/Wormhole/permissions/ComplexPermission.class */
enum ComplexPermission {
    ...
    static {
        for (ComplexPermission type : EnumSet.allOf(ComplexPermission.class)) {
            complexPermissionMap.put(type.complexPermissionNode, type);
        }
    }

    public static ComplexPermission fromComplexPermissionNode(String complexPermissionNode) {
        return complexPermissionMap.get(complexPermissionNode);
    }

    ComplexPermission(String complexPermissionNode) {
    ...
    public boolean checkPermission(Player player, Stargate stargate, String networkName) {
        ...
        if (player != null && WormholeXTreme.getPermissionManager() != null && !ConfigManager.getSimplePermissions()) {
            switch (this) {
                case NETWORK_USE:
                case NETWORK_BUILD:
                    allowed = networkName != null && WormholeXTreme.getPermissionManager().has(player, new StringBuilder().append(this.complexPermissionNode).append(networkName).toString());
                    break;
                case REMOVE_OWN:
                    ...
                    break;
                default:
                    allowed = WormholeXTreme.getPermissionManager().has(player, this.complexPermissionNode);
                    break;
            }
            if (allowed) {
